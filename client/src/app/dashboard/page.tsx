...
function DashboardContent() {
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center"><div className="text-center"><FiLoader className="text-blue-600 text-5xl mx-auto animate-spin mb-4" /><p className="text-gray-600 dark:text-gray-400">Loading...</p></div></div>}>
                  ...
                  <div className="relative w-20 h-20 sm:w-24 sm:h-24">
                    <svg className="w-20 h-20 sm:w-24 sm:h-24 transform -rotate-90">
                      ...
                      <circle
                        ...
                        strokeDashoffset={251.2 - (251.2 * Math.min(lectures.filter(l => {
                          const today = new Date().toDateString()
                          return new Date(l.created_at).toDateString() === today
                        }).length, 1)) / 1}
                        strokeLinecap="round"
      ...
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-[#1a2235]/90 backdrop-blur-2xl backdrop-saturate-150 border-t border-gray-200/80 dark:border-white/[0.08] z-50 pb-8 ${showOnboarding ? 'pointer-events-none opacity-50' : ''}`}>
        <div className="flex items-end justify-evenly h-16 px-4 pt-2">
          ...
          <div className="relative">
            ...
            disabled={isStoppingRecording || isGeneratingNotes || isTranscribing || showOnboarding}
