...
Remember: EVERY content line needs "- " at the start. NO exceptions.`
    ...
    const message = await anthropic.messages.create({
      model: options?.model || DEFAULT_CLAUDE_CONFIG.model,
      max_tokens: options?.maxTokens || DEFAULT_CLAUDE_CONFIG.maxTokens,
      messages: [
        {
          role: 'user',
    ...
    const notes = message.content[0].type === 'text' ? message.content[0].text : ''
    return notes
  } catch (error: any) {
    throw error
  }
